...
public class BranchRepository : GenericRepository<Branch>, IBranchRepository
{
    ...
    {
        return await _dbSet.Include(b => b.Projects).ToListAsync();
    }
...
public class ProjectRepository : GenericRepository<Project>, IProjectRepository
{
    ...
    {
        return await _dbSet.Include(p => p.Branch).Include(p => p.Tasks).ToListAsync();
    }

    public async Task<IEnumerable<Project>> GetProjectsByBranchAsync(int branchId)
    {
        return await _dbSet.Include(p => p.Branch).Include(p => p.Tasks).Where(p => p.BranchId == branchId).ToListAsync();
    }

    public override async Task<Project?> GetByIdAsync(int id)
    {
        return await _dbSet.Include(p => p.Branch).Include(p => p.Tasks).FirstOrDefaultAsync(p => p.Id == id);
    }
}
...
public class AppointmentRepository : GenericRepository<Appointment>, IAppointmentRepository
{
    ...
    {
        return await _dbSet.Include(a => a.Branch).Include(a => a.Project).FirstOrDefaultAsync(a => a.Id == id);
    }
}
